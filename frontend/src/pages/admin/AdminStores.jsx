...
const AdminStores = () => {
  ...
  const handleDelete = async (id, name) => {
    const confirmed = await confirmDelete(`Enter your administrator password to permanently delete the store "${name}". This action cannot be undone.`);
    if (!confirmed) return;
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="NS Store Colombo"
